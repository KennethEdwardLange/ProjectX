...
public class MergeSort {
    ...
    public ArrayList<Entity> mergeSort(ArrayList<Entity> whole) {
...
//        	if ( (left.get(leftIndex).compareTo(right.get(rightIndex))) < 0) {
            if ( (left.get(leftIndex).getY() + left.get(leftIndex).getHeight() < (right.get(rightIndex)).getY() + (right.get(rightIndex)).getHeight())) {
    ...
    public void start(ArrayList<Entity> input) {
        mergeSort(input);
    }
}